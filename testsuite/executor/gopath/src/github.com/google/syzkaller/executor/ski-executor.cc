...
static void execute_input();
static void execute_call_new(thread_t * th);
static thread_t* add_a_new_call(int call_index, int call_num, uint64 copyout_index, uint64 num_args, uint64* args, uint64* pos);
...
extern "C" char * ski_fetch_prog_data(int seed, int *size);
extern "C" char* ski_parse_env();
extern "C" int ski_test_start(int current_cpu, int total_cpus, int dry_run);
extern "C" void ski_test_finish(int current_cpu, int dry_run);
extern "C" void hypercall_debug(int current_cpu, char *format, ...);
extern "C" void hypercall_debug_quiet(int current_cpu, char *format, ...);
...
{	
	...
	// mlockall memory to avoid page faults
	...
	// Ask ski for input id
	int ski_ret = 0;
	if (STD_SKI_ENABLED == 1)
		ski_ret = ski_test_start(STD_SKI_CPU_AFFINITY, STD_SKI_TOTAL_CPUS, 0);
	else
		hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, "SKI is disabled and interfaces are not used\n");
	ski_ret = ski_ret >> 2;//move bit
	// If the input id is 1, then end the program immediately because this is snapshotting mode
	if (ski_ret == 1){
		hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, "[set_syscal_nr CPU%d] ski_start return 1, for SNAPSHOT use\n", STD_SKI_CPU_AFFINITY);
		if (STD_SKI_ENABLED == 1)
	...
	// Apply for test input
	prog_data = ski_fetch_prog_data(ski_ret, &size);
	if (size == 0){
		if (STD_SKI_ENABLED == 1){
			hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"No program data is fetched");
			ski_test_finish(STD_SKI_CPU_AFFINITY,0);
		}
		doexit(0);
	}
	// Pass program data to input_data
	for (index=0; index < size; index++){
		input_data[index] = *(prog_data+index);
	}
	// Actually execute the sequential input
	execute_input();
	// finish the test
	if (STD_SKI_ENABLED == 1)
		ski_test_finish(STD_SKI_CPU_AFFINITY,0);
...
void execute_input(){
	...
	for (;;) {
		...
		if (call_num >= ARRAY_SIZE(syscalls))
			fail("invalid command number %llu %llu", call_num, ARRAY_SIZE(syscalls));
		...
		for (uint64 i = num_args; i < kMaxArgs; i++)
			args[i] = 0;
		// Prepare to execute the syscall
		thread_t* th = add_a_new_call(call_index++, call_num, copyout_index,
					     num_args, args, input_pos);
		execute_call_new(th);
	}
}

void execute_call_new(thread_t* th){
	const call_t* call = &syscalls[th->call_num];
	if (STD_SKI_ENABLED)
		hypercall_debug(STD_SKI_CPU_AFFINITY, "Call %s Call_NUM: %d NUM_Args: %d \n", call->name, call->sys_nr, th->num_args);
	else
		printf("Call %s Call_NUM: %d NUM_Args: %d \n", call->name, call->sys_nr, th->num_args);
	for (int i = 0; i < th->num_args; i++) {
		if (STD_SKI_ENABLED)
			hypercall_debug(STD_SKI_CPU_AFFINITY, "Args%d=0x%llx\n", i, (uint64)th->args[i]);
		else
			printf("Args%d=0x%llx\n", i, (uint64)th->args[i]);
	}
	...
	if ((th->res == -1 && th->reserrno == 0) || call->attrs.ignore_return)
		th->reserrno = EINVAL;
	if (STD_SKI_ENABLED)
		hypercall_debug(STD_SKI_CPU_AFFINITY, "%s=0x%llx errno=%d\n", call->name, (uint64)th->res, th->reserrno);
	else
		printf("%s=0x%llx errno=%d\n", call->name, (uint64)th->res, th->reserrno);
	if (th->res != (intptr_t)-1)
		copyout_call_results(th);
}


thread_t* add_a_new_call(int call_index, int call_num, uint64 copyout_index, uint64 num_args, uint64* args, uint64* pos)
{
