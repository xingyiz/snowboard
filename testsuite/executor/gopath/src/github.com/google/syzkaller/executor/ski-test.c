...
hypercall_io hio_data;

char* ski_fetch_prog_data(int seed, int *size){
    hypercall_io_init(&hio_data);
    hio_data.hypercall_type = HYPERCALL_IO_TYPE_FETCH_DATA;
    hio_data.p.hio_fetch_data.seed = seed;
    if(STD_SKI_HYPERCALLS){
        hypercall(&hio_data);
		printf("program data is stored at %08x\n", hio_data.p.hio_fetch_data.progdata);
        //for (int index=0; index<(hio_data.p.hio_fetch_data.size); index++){
            //printf("%08x %u\n", hio_data.p.hio_fetch_data.progdata + index, *(hio_data.p.hio_fetch_data.progdata + index));
        //  }
        printf("program size is %d\n",  hio_data.p.hio_fetch_data.size);
        *size = hio_data.p.hio_fetch_data.size;
        return &hio_data.p.hio_fetch_data.progdata;
    }
    return 0;
}
...
static int ski_thread_start(int thread_num, int total_cpus, int dry_run){
    ...
    if(with_barriers){
//		shared_printf("[%d] [CPU%d] Going into usermode barrier\n", thread_num, STD_SKI_CPU_AFFINITY);
...
//		shared_printf("[%d] Passed the usermode barrier\n", thread_num ); //XXX: Should take this out...
	...
	//shared_printf("[%d] what happened, this process is beyond barrier\n");
...
void ski_trinity_barrier(int current_cpu){
    trinity_barrier = shared_malloc(sizeof(barrier_t), "TRINITY_barrier", current_cpu);
    if (current_cpu == 0){
        shared_printf("[CPU%d] Initialize trinity barrier\n", STD_SKI_CPU_AFFINITY);
        barrier_init(trinity_barrier, 4);
    }
    sleep(10);
    return;
...
void ski_wait_trinity(){
    barrier_wait(trinity_barrier);
    shared_printf("passed the trinity barrier\n");
...
void ski_tests_init(int current_cpu, int total_cpus){
    ...
    if(with_barriers || STD_SKI_SOFT_EXIT_BARRIER){
        ...
        shared_printf("[ski_tests_init CPU%d] barrier addr=%08X\n", STD_SKI_CPU_AFFINITY, usermode_barrier);
	    if(current_cpu == 0){
			barrier_init(usermode_barrier, total_cpus);
		}

		shared_printf("[ski_tests_init CPU%d] Sleeping for 10 seconds.\n", STD_SKI_CPU_AFFINITY);
		sleep(10);
	}
    return;
}
