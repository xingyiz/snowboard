...
int barrier_init(barrier_t *barrier,int needed)
{
	...
	if(with_fork){
		...
		if(pthread_mutexattr_setpshared(&mattr, PTHREAD_PROCESS_SHARED)){
			...
			shared_printf("Mutex PTHREAD_PROCESS_SHARED\n");
			exit(1);
		}
		...
		if(pthread_condattr_setpshared(&cattr, PTHREAD_PROCESS_SHARED)){
			...
			shared_printf("Cond PTHREAD_PROCESS_SHARED\n");
			exit(1);
		}

		pthread_mutex_init(&barrier->mutex,&mattr);
	    pthread_cond_init(&barrier->cond, &cattr);
		shared_printf("mutx and cond are created\n");
	}else{
...
int barrier_wait(barrier_t *barrier)
{
	...
	barrier->called++;
	shared_printf("A new process reaches the barrier, called=%d needed=%d\n", barrier->called, barrier->needed); 
