...
var (
	...
	flagSave      = flag.String("save", "", "save place")
...
func (ctx *Context) execute(pid int, env *ipc.Env, entry *prog.LogEntry) {
	...
	var progdata []byte
    progdata = make([]byte, 4 << 21)
    progSize, err := entry.P.SerializeForExec(progdata)
	if err != nil {
		fmt.Println("Seirlia  aeerreerr",err.Error())
        os.Exit(2)
	}
